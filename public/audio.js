...
/**
 * @class CodeGenCityRustGenerator
 * @extends AudioWorkletProcessor
 */
class CodeGenCityRustGenerator extends AudioWorkletProcessor {
  constructor() {
    // @ts-check
    super();
    /**
    ...
    this.uiWaveformView = {
      ...
      counter: 0,
      shouldSend: nSamples =>
        (this.uiWaveformView.counter += nSamples) >=
        this.uiWaveformView.sampleRate,
      post: () => {
        sendWaveformData(this.uiWaveformView.buffer, port);
        this.uiWaveformView.counter = 0;
      }
    };
  }
      ...
      if (globalThis.synth) {
        ...
        if (this.uiWaveformView.shouldSend(output[channel].length)) {
          output[channel]
            ...
            .forEach(
              (sample, idx) => (this.uiWaveformView.buffer[idx] = sample)
            );
...
registerProcessor('city-rust', CodeGenCityRustGenerator);
...
function loadPreset(data) {
  ...
  if (synth && typeof data === 'string') {
    console.warn(
      'changing from current preset.. ' + synth.print('preset name: ')
    );
...
function dumpAllParam(data, port) {
  const { dumpAll } = data;
  const paramIdens = Object.keys(Param).map(param => ({
    param: param,
    paramIden: Param[param]
  }));
  ...
  if (synth && dumpAll === true) {
    const values = paramIdens.map(item => ({
      ...item,
      value: synth.get_state(item.paramIden)
    }));
    const dump = values.reduce(
    ...
    ['./citysynth_wasm']: {
      ['__wbindgen_throw']: (i1, i2) => {
        try {
          ...
          console.error(
            'throw wasm error' + ex ? `: ${JSON.stringify(ex)}` : ''
          );
        }
      }
    }
...
class CitySynth {
  ...
  static new(arg0, arg1) {
    return CitySynth.__wrap(
      wasm.citysynth_new(arg0, !isLikeNone(arg1), isLikeNone(arg1) ? 0 : arg1)
    );
  }
  /**
  ...
  note_on(arg0, arg1) {
    return wasm.citysynth_note_on(
      this.ptr,
      arg0,
      !isLikeNone(arg1),
      isLikeNone(arg1) ? 0 : arg1
    );
  }
  /**
   * @param {number} arg0
  ...
  set_delay_reverb_active(arg0) {
    return wasm.citysynth_set_delay_reverb_active(this.ptr, arg0);
  }
  /**
   * @param {number} arg0
   * @returns {number}
